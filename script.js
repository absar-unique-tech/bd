  ...
  grid.innerHTML = products.length?products.map(p=>`
    <div class="p-card">
      <div class="p-img" onclick="window.location='product.html?id=${p.id}'">
        <img src="${(p.images||[])[0] || p.img}" alt="${p.name}" onerror="this.src='https://via.placeholder.com/300'">
...
window.openOrderForm = function(p){
  ...
  document.getElementById('orderColor').value = (p.colors||[])[0] || (p.color? p.color.split(',')[0].trim() : 'N/A');
...
window.submitOrder = function(){
  ...
  const deliveryArea = deliveryCharge === 80
