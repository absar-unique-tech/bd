...
async function loadProducts(){
  try {
    ...
    document.getElementById('grid').innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:40px;color:red">Error: '+e.message+'</div>';
  }
}
...
function render(products){
  ...
  grid.innerHTML = products.length?products.map(p=>{
    ...
    const hasVideo = (p.images||[]).some(u=>u.includes('youtube') || u.includes('youtu.be'));
    ...
    <div class="p-card">
      ...
      <div class="p-info">
        ...
        <div style="display:flex;gap:8px;margin-top:auto">
          <button class="buy-btn" style="flex:1;background:#666" onclick='addToCart(${JSON.stringify(p)})' ${p.soldout || p.stock === 0?'disabled':''}>
            ${p.soldout || p.stock === 0?'Out of Stock':'🛒 Cart'}
          </button>
          <button class="buy-btn" style="flex:1" onclick='openOrderForm(${JSON.stringify(p)})' ${p.soldout || p.stock === 0?'disabled':''}>
            Buy Now
          </button>
        </div>
      </div>
